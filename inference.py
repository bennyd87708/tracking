...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for i in variables:
        ...
        if i in [GHOST0, PAC, GHOST1]:
            for a in range(X_RANGE):
                for b in range(Y_RANGE):
                    variableDomainsDict[i].append((a, b))
        else:
            variableDomainsDict[i] = [x for x in range(MAX_NOISE + X_RANGE + Y_RANGE - 1)]
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        cptswe = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for hv in eliminationOrder:
            cptswe, factor = joinFactorsByVariable(cptswe, hv)
            if len(factor.unconditionedVariables()) > 1:
                cptswe.append(eliminate(factor, hv))
        return normalize(joinFactors(cptswe))
        "*** END YOUR CODE HERE ***"
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        phantomlord = random.random() * self.total()
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if ghostPosition == jailPosition or noisyDistance is None:
            return 1 if ghostPosition == jailPosition and noisyDistance is None else 0
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            ...
            for pair in newPosDist.items():
                newBeliefs[pair[0]] += self.beliefs[oldPos] * pair[1]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        posPerParticle = float(len(self.legalPositions)) / self.numParticles
        score = 0
        for p in self.legalPositions:
            score += 1
            while score > posPerParticle:
                score -= posPerParticle
